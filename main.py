...
class ImageViewer(QGraphicsView):
    ...
    def load_map_sizes(self):
        ...
        if hasattr(self, 'current_map'):
            if self.current_map == 'malden':
                filename = os.path.join(self.json_directory, 'map_sizes_malden.json')
            elif self.current_map == 'altis_amazing':
                filename = os.path.join(self.json_directory, 'map_sizes_altis_amazing.json')
            else:
                filename = os.path.join(self.json_directory, 'map_sizes_altis.json')
        else:
            filename = os.path.join(self.json_directory, 'map_sizes_altis.json')
        try:
    ...
    def load_points_from_json(self):
        ...
        for json_file in os.listdir(self.json_directory):
            if json_file.endswith('.json') and json_file != 'local_player.json' and json_file not in (
                    'map_sizes_altis.json', 'map_sizes_malden.json', 'map_sizes_altis_amazing.json'):
                try:
        ...
        try:
            ...
            for point in data['points']:
                ...
                if not (0 <= x_scaled <= pixmap.width() and 0 <= y_scaled <= pixmap.height()):
                    ...
                    continue
                color = self.get_color_by_type(point.get('type', 1))
    ...
    def render_local_player(self, player_info):
        ...
        color = self.get_color_by_type(player_info.get('type', 1))
        self.add_dot(x_scaled, y_scaled, color)
    ...
    def get_color_by_type(self, point_type):
        color_map = {
    ...
    def draw_view_line(self, x, y, view_x, view_y, view_z, is_local=False):
        length = 8000 if is_local else 100
        try:
            view_x = float(view_x)
            view_y = float(view_y)
            view_z = float(view_z)
...
if __name__ == '__main__':
    ...
    viewer = ImageViewer(
        image_path='C:\\Arma3\\altis.png',
        json_directory='C:\\Arma3\\'
    )
   ...
   # checkbox_violet = QCheckBox("Violet (coords3)")
    checkbox_transport = QCheckBox("Show Transport")
    checkbox_transport.setChecked(True)
    toggle_button = QPushButton("Show Name")
   # for cb in [checkbox_red, checkbox_blue, checkbox_green, checkbox_violet]:
    ...
    def switch_to_altis():
        ...
        new_pixmap = QPixmap('C:\\Arma3\\altis.png')
    ...
    def switch_to_malden():
        viewer.current_map = 'malden'
        viewer.load_map_sizes()
        new_pixmap = QPixmap('C:\\Arma3\\malden.png')
        viewer.pixmap_item.setPixmap(new_pixmap)
        pixmap = new_pixmap
        viewer.setSceneRect(-pixmap.width(), -pixmap.height(), 3 * pixmap.width(), 3 * pixmap.height())
        viewer.load_points_from_json()

    def switch_to_altis_amazing():
        viewer.current_map = 'altis_amazing'
        viewer.load_map_sizes()
        new_pixmap = QPixmap('C:\\Arma3\\altis_amazing.png')
        viewer.pixmap_item.setPixmap(new_pixmap)
        pixmap = new_pixmap
        viewer.setSceneRect(-pixmap.width(), -pixmap.height(), 3 * pixmap.width(), 3 * pixmap.height())
        viewer.load_points_from_json()

    btn_altis.clicked.connect(switch_to_altis)
    btn_malden.clicked.connect(switch_to_malden)
    btn_altis_amazing.clicked.connect(switch_to_altis_amazing)
...
#
   ...
   #     controls_layout.addWidget(cb)
    ...
    controls_layout.addWidget(btn_altis)
    controls_layout.addWidget(btn_malden)
